      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {eventTypes.length > 1 && (
          <div className="border-b border-gray-100 p-4 space-y-2">
            <p className="mb-3 text-sm font-medium text-gray-700">ประเภทการจอง</p>
            {eventTypes.map((et) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-4 rounded-2xl border-2 p-4 text-left transition active:scale-[0.98]
                    ${isSelected ? 'border-transparent shadow-sm' : 'border-gray-100 bg-white hover:border-gray-200'}`}
                  style={isSelected ? { backgroundColor: et.color || '#3B82F6' } : {}}
                >
                  <div
                    className="h-10 w-10 shrink-0 rounded-xl"
                    style={{ backgroundColor: isSelected ? 'rgba(255,255,255,0.25)' : (et.color || '#3B82F6') + '22' }}
                  >
                    <div className="flex h-full items-center justify-center">
                      <div
                        className="h-3 w-3 rounded-full"
                        style={{ backgroundColor: isSelected ? '#fff' : (et.color || '#3B82F6') }}
                      />
                    </div>
                  </div>
                  <div className="min-w-0 flex-1">
                    ...
                    {et.description && (
                      <p className={`mt-0.5 text-xs line-clamp-1 ${isSelected ? 'text-white/75' : 'text-gray-400'}`}>
                        {et.description}
                      </p>
                    )}
                  </div>
