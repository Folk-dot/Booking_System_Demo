...
import { toBkkDateStr, formatTime, formatDate } from '@/shared/utils/dateUtils.js';
...
export default function SlotPicker() {
  ...
  useEffect(() => {
    ...
    liffApi
      .get('/slots', { params: { trainer_id: trainerId, date: dateStr } })
  ...
  function handleSelectSlot(slot) {
    navigate('/confirm', { state: { slot, trainer } });
  }
  ...
  return (
    <div className="flex h-screen flex-col">
      ...
      <div className="border-b border-gray-100 p-4">
        ...
        {trainer?.specialty && (
          <p className="text-sm text-brand-600">{trainer.specialty}</p>
        )}
      </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 gap-3">
            {slots.map((slot) => (
              ...
              >
                <p className="text-lg font-bold text-gray-900">{formatTime(slot.startsAt)}</p>
                <p className="text-xs text-gray-400">ถึง {formatTime(slot.endsAt)}</p>
              </button>
            ))}
          </div>
        )}
